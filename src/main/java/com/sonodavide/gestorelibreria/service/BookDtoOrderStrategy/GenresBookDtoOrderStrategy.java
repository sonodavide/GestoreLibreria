...
public class GenresBookDtoOrderStrategy implements BookDtoOrderStrategy {


    @Override
    public List<BookDto> execute(List<BookDto> bookDtos, boolean reverse) {
        Comparator<BookDto> comparator = new GenresComparator();
        if (reverse){
            comparator = comparator.reversed();
        }
        return bookDtos.stream().sorted(comparator).collect(Collectors.toList());
    }
    class GenresComparator implements Comparator<BookDto> {

        @Override
        public int compare(BookDto b1, BookDto b2) {
            return compareSortedStrings(b1.getGenres(), b2.getGenres());
        }
