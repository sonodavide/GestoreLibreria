...
public class IsbnBookDtoFilterStrategy implements BookDtoFilterStrategy {

    @Override
    public List<BookDto> execute(List<BookDto> bookDtos, String param) {
        String paramLC = param.toLowerCase();
        return bookDtos.stream().filter(bookDto -> bookDto.getIsbn().toLowerCase().startsWith(paramLC) ).collect(Collectors.toList());
    }
}
