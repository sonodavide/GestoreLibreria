...
public class PublisherBookDtoFilterStrategy implements BookDtoFilterStrategy {
    @Override
    public List<BookDto> execute(List<BookDto> bookDtos, String param) {
        String paramLC = param.toLowerCase();
        return bookDtos.stream().filter(bookDto -> bookDto.getPublisher().toLowerCase().startsWith(paramLC)).collect(Collectors.toList());
    }
}
